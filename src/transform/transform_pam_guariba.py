...
ARQUIVO_ENTRADA = Path(
    "data/raw/agricola/pam_cana_guariba_raw.csv"
)
...
ARQUIVO_SAIDA_CSV = Path(
    "data/processed/pam_cana_guariba_anual.csv"
)
...
def converter_valor(valor):
    ...
    # Caso exista ponto como separador de milhar e vírgula decimal.
...
def carregar_dados() -> pd.DataFrame:
    """
    Carrega os dados brutos coletados da API SIDRA.
    """
...
def transformar_dados(df: pd.DataFrame) -> pd.DataFrame:
    """
    Filtra, padroniza, reorganiza e calcula indicadores agrícolas.
    """

    # Mantém apenas Guariba e as variáveis relevantes ao projeto.
    df = df[
        (df["D1C"] == CODIGO_GUARIBA)
        & (df["D2C"].isin(MAPA_VARIAVEIS))
    ].copy()
    ...
    return df_wide.sort_values("ano").reset_index(drop=True)
...
def main():
    print("=" * 60)
    print("ETAPA 4 — TRANSFORMAÇÃO DOS DADOS PAM / GUARIBA-SP")
    print("=" * 60)
